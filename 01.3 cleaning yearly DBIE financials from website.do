...
Users: Tejas, Nirupama
--------------------------------------*/

/* not useful - import excel "$raw/_1.Liabilities and Assets of Scheduled Commercial Banks .xlsx", cellrange(B7) firstrow sheet("ASSETS")  clear */
...
foreach var of varlist _all {
     ...
     // Step 3: remove other invalid characters
     local clean = ustrregexra("`clean'", "[^a-zA-Z0-9_]", "")
 
     // Step 4: strip leading non-letters
     local clean = ustrregexra("`clean'", "^[^a-zA-Z]+", "")
  
    // Step 5: truncate to 32 characters max
...
// Standardize bank names for perfect merge
replace bank_name = "AXIS BANK LTD." if bank_name == "AXIS BANK LIMITED"
replace bank_name = "BANDHAN BANK LTD." if bank_name == "BANDHAN BANK LIMITED"
replace bank_name = "CATHOLIC SYRIAN BANK LTD." if bank_name == "CSB BANK LIMITED"
replace bank_name = "CITY UNION BANK LTD." if bank_name == "CITY UNION BANK LIMITED"
replace bank_name = "DCB BANK LTD." if bank_name == "DCB BANK LIMITED"
replace bank_name = "DHANALAKSHMI BANK LTD." if bank_name == "THE DHANALAKSHMI BANK LTD" | bank_name == "DHANLAXMI BANK LIMITED"
replace bank_name = "FEDERAL BANK LTD." if bank_name == "FEDERAL BANK LTD"
replace bank_name = "ICICI BANK LTD." if bank_name == "ICICI BANK LIMITED"
replace bank_name = "IDFC BANK LTD." if bank_name == "IDFC FIRST BANK LIMITED"
replace bank_name = "ING VYSYA BANK LTD." if bank_name == "ING VYSYA BANK" | bank_name == "ING VYSYA BANK LTD"
replace bank_name = "JAMMU AND KASHMIR BANK LTD." if bank_name == "JAMMU & KASHMIR BANK LTD"
replace bank_name = "KARNATAKA BANK LTD." if bank_name == "KARNATAKA BANK LTD"
replace bank_name = "KOTAK MAHINDRA BANK LTD" if bank_name == "KOTAK MAHINDRA BANK LTD."
replace bank_name = "LAKSHMI VILAS BANK LTD." if bank_name == "LAKSHMI VILAS BANK LTD"
replace bank_name = "RBL BANK LTD" if bank_name == "RBL BANK LIMITED"
replace bank_name = "SOUTH INDIAN BANK LTD." if bank_name == "SOUTH INDIAN BANK LTD"
replace bank_name = "TAMILNADU MERCANTILE BANK LTD." if bank_name == "TAMILNAD MERCANTILE BANK LTD"

// Now merge with cleaned bank name reference file
merge m:1 bank_name using "$clean/bank_names_updated", force nogen keep(3)

save "$raw/DBIE_2024_Assets", replace 
...
save "$raw/DBIE_2019_Liabilities", replace 
...
save "$raw/DBIE_2020_Liabilities", replace 

// Now appending the liabilities together and merging with the assets
use "$raw/DBIE_2019_Liabilities", clear 
append using "$raw/DBIE_2020_Liabilities", force // doesn't have data on investment fluctuation reserves 
sort bank_name year 

merge 1:1 bank_name year using "$raw/DBIE_2024_Assets", force nogen keep(3)


destring year, replace
save "$clean/DBIE_financials_2005_2024_file_1", replace
...
// Year Banks aInterestDiscountearnedon bIncomeoninvestments cInterestonbalanceswithR dOthers TotalI aCommissionexchangeandbr bNetprofitlossonsaleo cNetprofitlossonrevalu dNetprofitlossonsaleo eNetprofitlossonexchan fMiscellaneousincome TotalII TotalIII
...
save "$raw/DBIE_2024_Income", replace 
...
save "$raw/DBIE_2024_Expenses", replace 

// merging the two files 

merge 1:1 bank_name year using "$raw/DBIE_2024_Income", force nogen keep(3)


destring year, replace

save "$clean/DBIE_financials_2005_2024_file_2", replace 
...
save "$clean/DBIE_financials_2005_2024_file_4", replace 
...
save "$clean/DBIE_financials_2005_2024_file_3", replace 
...
/* Renaming for the merges // need to discuss
replace bank_name = "BANK OF BARODA" if bank_name == "BANK OF BARODA*"
replace bank_name = "CANARA BANK" if bank_name == "CANARA BANK*"
replace bank_name = "INDIAN BANK" if bank_name == "INDIAN BANK*"
replace bank_name = "PUNJAB NATIONAL BANK" if bank_name == "PUNJAB NATIONAL BANK*"
replace bank_name = "STATE BANK OF INDIA" if bank_name == "STATE BANK OF INDIA*"
replace bank_name = "UNION BANK OF INDIA" if bank_name == "UNION BANK OF INDIA*"
*/
...
save "$clean/DBIE_financials_2005_2024_file_6", replace 
...
save "$clean/DBIE_financials_2005_2024_file_8", replace 
...
// NetProfit ProfitBroughtForward ProfitAvailableforAppropriati TransfertoStatutoryReserves TransfertoCapitalReserves TransfertodebentureRedemption TransfertoInvestmentsFluctuat TransfertoOtherReserves TransfertoProposedDividend TransfertoTaxonDividend BalanceCarriedOvertoBalance
...
save "$clean/DBIE_financials_2005_2024_file_7", replace 
...
drop in 1
// B C a114days b1528days c29daysto3months dOver3monthsto6months eOver6monthsto1year fOver1yearto3years gOver3yearsto5years hOver5years TotalofDeposits M N O P Q R S T TotalOfBorrowings V W X Y Z AA AB AC TotalofInvestmentsatbookva AE AF AG AH AI AJ AK AL TotalofForeignCurrencyAssets AN AO AP AQ AR AS AT AU TotalofForeignCurrencyLiabil AW AX AY AZ BA BB BC BD TotalofLoansandAdvances
...
save "$clean/DBIE_financials_2005_2024_file_9", replace 
...
drop in 1
//Ratioofinvestmentsinnon Ratioofinterestincometo Ratioofnetinterestincom Ratioofnoninterestincom Ratioofintermediationcos Ratioofwagebillstointe Ratioofwagebillstotota T Ratioofburdentototalas Ratioofburdentointerest Ratioofoperatingprofits Returnonassets Returnonequity Costofdeposits Costofborrowings Costoffunds Returnonadvances Returnoninvestments Returnonadvancesadjusted Returnoninvestmentsadjus BusinessperemployeeinRu ProfitperemployeeinRu Capitaladequacyratio CapitaladequacyratioTie AK RatioofnetNPATonetadv
...
rename Ratioofnetinterestincom NIM // label the formula to this later 
...
**# Need to add the replace code for 2024
replace year = "2024" if missing(year) | year == "." | year == "" 


// Now merge with cleaned bank name reference file
merge m:1 bank_name using "$clean/bank_names_updated", force nogen keep(3) // 821 observations unlike the usual 822 

destring year, force replace
save "$clean/DBIE_financials_2005_2024_file_11", replace 
...
drop in 1
// Year Bank StandardAssetsduringtheYear SubStandardAssetsduringtheY DoubtfulAssetsduringtheYear Total1123 H I J Total2567 Total348
...
destring year, replace
save "$clean/DBIE_financials_2005_2024_file_13", replace 




import excel "$raw/12.Bank-wise and Category-wise Employees of Scheduled Commercial Banks.xlsx", cellrange(B4) firstrow clear 


// Year BankName Officers Clerks SubStaff Total

rename Year year 
rename BankName bank_name 
rename Officers Total_No_Officers 
rename Clerks Total_No_Clerks 
rename SubStaff Total_No_Sub_Staff
rename Total Total_Employees

drop if mi(bank_name)

// forward setting the year 
* Step 2: Extract the first 4 digits (e.g., 2005)
gen base_year = real(substr(year, 1, 4))

* Step 3: Extract the last 2 digits (e.g., 06)
gen suffix = real(substr(year, -2, 2))

* Step 4: If suffix < 50, then assume it's 2000s; else handle 1900s if needed
gen year_clean = base_year + 1  // since 2005-06 → 2006

* Step 5: Replace original variable if you want
drop base_year suffix year
...
destring year, replace
save "$clean/DBIE_financials_2005_2024_file_12", replace


// Merging all the sheets together to get the 2024 master financials 


use "$clean/DBIE_financials_2005_2024_file_1", clear 

forvalues i = 2/13 {
	local f "$clean/DBIE_financials_2005_2024_file_`i'.dta" // need this for the confirm capture and we don't have file 5,10, 12 
...
save "$clean/Annual_Financals_DBIE_2005_2024", replace 



use "$raw/branch_stock_final", clear 

// fixing the month and year variable 
//Convert string to Stata daily date
gen daily_date = daily(dateofopening, "DMY")
format daily_date %td

//Extract month and calendar year
gen month = month(daily_date)
gen year = year(daily_date)

//Derive financial year Apr–Mar
gen fin_year = year + (inrange(month, 4, 12))

//Optionally, show as yearly-financial label
format fin_year %ty

collapse (count) srno, by(bankname fin_year)
...
// ALLAHABAD BANK, ANDHRA BANK, CORPORATION BANK, DENA BANK, LAKSHMI VILAS BANK LTD., ORIENTAL BANK OF COMMERCE, STATE BANK OF BIKANER AND JAIPUR, STATE BANK OF HYDERABAD, STATE BANK OF MYSORE, STATE BANK OF PATIALA, STATE BANK OF TRAVANCORE, SYNDICATE BANK, UNITED BANK OF INDIA, VIJAYA BANK - 32 banks left 

// Now merge with cleaned bank name reference file after getting the banks we don't have 
preserve 
merge m:1 bank_name using "$clean/bank_names_updated", force nogen keep(2) 
keep bank_name 
duplicates drop 
save "$clean/bank_names_unmatched_branches", replace
restore 

merge m:1 bank_name using "$clean/bank_names_updated", force nogen keep(3) 
sort bank_name fin_year
bysort bank_name (fin_year): gen Total_Bank_Branches = sum(srno) // cumalative term 
rename srno Addition_Bank_Branches
rename fin_year year
la var Total_Bank_Branches "Total Amount of Bank Branches for that year"
la var Addition_Bank_Branches "Total Amount of Bank Branches Added in that year"


// saving the complete list of banks across the years 

save "$clean/bank_branches_data_all_years", replace 

drop if year < 2005

save "$clean/bank_branches_data_2005", replace 
...
// Getting the bank branch values for banks we don't have values for 

use "$raw/Open Bank Branches", clear 
sort bank_name year 
replace bank_name = "AXIS BANK LTD." if bank_name == "AXIS BANK LIMITED"
replace bank_name = "BANDHAN BANK LTD." if bank_name == "BANDHAN BANK LIMITED"
replace bank_name = "CATHOLIC SYRIAN BANK LTD." if bank_name == "CSB BANK LIMITED"
...
merge m:1 bank_name using "$clean/bank_names_updated", force nogen keep(3) 
keep bank_name year total_
drop if missing(bank_name)
destring total_, replace 
replace total_ = 0 if missing(total_)
rename total_ number_of_banks_opened
sort bank_name year 
save "$clean/number_of_open_branches", replace 


use "$raw/Closed Bank Branches", clear 
sort bank_name year 
replace bank_name = "AXIS BANK LTD." if bank_name == "AXIS BANK LIMITED"
replace bank_name = "BANDHAN BANK LTD." if bank_name == "BANDHAN BANK LIMITED"
replace bank_name = "CATHOLIC SYRIAN BANK LTD." if bank_name == "CSB BANK LIMITED"
...
destring total_, replace 
replace total_ = 0 if missing(total_)
rename total_ number_of_banks_closed
sort bank_name year 
save "$clean/number_of_closed_branches", replace 

merge 1:1 bank_name year using "$clean/number_of_open_branches", force nogen 
ge net_addition_bank_branches = number_of_banks_opened - number_of_banks_closed
save "$clean/net_addition_bank_branches", replace 



// reworking this part because advait's file doesn't have all the banks we need 

use "$raw/num_branches", replace 
drop part1cd
duplicates drop
...
save "$clean/bank_branches_data_till_2016_Debargah_source", replace 

merge m:1 bank_name using "$clean/bank_names_updated", force nogen keep(3) 
merge 1:1 bank_name year using "$clean/net_addition_bank_branches", force 
sort bank_name year 
encode bank_name, gen(bankcd)
xtset bankcd year 
replace tot_branches = net_addition_bank_branches + l.tot_branches if tot_branches ==. 
drop _merge 


merge 1:1 bank_name year using "$clean/bank_branches_data_2005", force
replace tot_branches = Total_Bank_Branches if tot_branches == .
drop if _merge == 2 
sort bank_name year 
xtset bankcd year 
replace tot_branches = net_addition_bank_branches + l.tot_branches if tot_branches ==. & bank_name == "CATHOLIC SYRIAN BANK LTD."

save "$clean/final_bank_branches_data_2005", replace 

keep year bank_name tot_branches 
**# Need to rework the totaling for the mergers 
merge 1:1 bank_name year using "$clean/Annual_Financals_DBIE_2005_2024", force nogen keep(2 3)
save "$clean/Annual_Financals_DBIE_2005_2024_Bank_Branches_v1", replace 


**# Bank Closure Data IF NEEDED 

/*
// cleaning the closure bank branches data (gives us reasons for shutting down etc)
import excel "$raw/closed_branches_ifsc.xlsx", firstrow clear 
rename BankName bank_name 
...
// removing banks that don't match 

merge m:1 bank_name using "$clean/bank_names_updated", force nogen keep(3)



// creating a financial year variable - the BranchCloseDate is already a date 

gen mnth = mofd(BranchCloseDate)  // Will get a mnth value such as 2020m4
format mnth %tm

gen month_number = month(dofm(mnth))
gen year = year(dofm(mnth))

replace year = year + 1 if inrange(month_number,4,12)

drop if bank_name == NewBankName
drop if CloseReason == "CONVERSION"  */
...
merge 1:1 bank_name year using "$clean/annual_bank_financals_DBIE_2002_2023", force keepusing (NRI_Deposits Certificates_Deposits Tier_I_Capital_Standalone CET_I_Capital_Standalone Tier_II_Capital_Standalone Total_Capital_Standalone Total_RWAs_Standalone_a_b_c Tot_RWAs_Cred_Risk_Standalone_a Tot_RWAs_MKt_Risk_Standalone_b Tot_RWAs_Oprt_Risk_Standalone_c Capital_Ratio_CRAR_Standalone Food_Credit_Gross_Adv - Other_Priority_Sect_GNPA_Ratio) nogen keep(1 3)
...
/* Another way of running that loop 
// Get all variables except the ones we want to keep as strings
ds (year bank_name bank_id), not
local to_destring `r(varlist)'

// Loop over and destring the rest
foreach v of local to_destring {
...
ds (year bank_name tot_branches _merge6 Cash_Deposit_Ratio Credit_Deposit_Ratio Inv_Deposit_Ratio Cred_and_Inv_to_Deposit_Ratio Dep_to_Total_Lia_Ratio Dem_and_Sav_Dep_to_Total_Dep PSL_Loans_to_Total_Loans_Ratio Term_Loans_to_Total_Adv_Ratio Sec_Adv_to_Total_Adv_Ratio Non_Approved_Sec_Inv_to_Tot_Inv Int_Inc_to_Total_Assets_Ratio NIM Non_Int_Inc_to_Tot_Assets_Ratio Intermediation_cost_to_Tot_Asset Wage_Bill_to_Intermediation_cost Wage_Bill_to_Tot_Expense_Ratio Wage_Bill_to_Total_Income_Ratio Ratioofburdentototalas Ratio_of_burden_to_int_income Operating_Profit_to_Tot_Assets Return_on_Assets Return_on_Equity Cost_of_Deposits Cost_of_Borrowings Cost_of_Funds Return_on_Advances Return_on_Investments Return_on_Advances_Adjusted Return_on_Inv_Adjusted Business_per_emp_in_Rs_Lakhs Profit_per_emp_in_Rs_Lakhs Capital_Adequacy_Ratio Capital_Adequacy_Ratio_TierI Capital_Adequacy_Ratio_TierII Net_NPA_to_Total_Adv_Ratio Total_No_Officers Total_No_Clerks Total_No_Sub_Staff Total_Employees Capital_Ratio_CRAR_Standalone Food_Credit_GNPA_Ratio Non_Food_Credit_GNPA_Ratio Agri_Allied_Act_GNPA_Ratio Industry_GNPA_Ratio Services_GNPA_Ratio Retail_Loans_GNPA_Ratio Other_Non_Food_Cred_GNPA_Ratio Retail_Housing_Loans_GNPA_Ratio Retail_Auto_Loans_GNPA_Ratio Retail_Credit_Cards_GNPA_Ratio Retail_Edu_Loans_GNPA_Ratio Other_Retail_Loans_GNPA_Ratio Gross_NPAs_to_Gross_Adv_perc_p Priority_Sect_GNPA_Ratio Non_Priority_GNPA_Ratio Priority_Agri_Allied_GNPA_Ratio Priority_MSE_GNPA_Ratio Other_Priority_Sect_GNPA_Ratio bank_id), not
...
// Tring to relabel some variables didn't work

/*
set trace on 

local vars capital total_reserves_and_surplus statutory_reserves capital_reserves share_premium investments_fluctuations_reserve revenue_and_other_reserves balance_of_profit total_deposits total_demand_deposits demand_deposits_from_banks demand_deposits_from_others total_saving_deposits total_term_deposits term_deposits_from_banks term_deposits_from_others Deposits_from_indian_branches Deposit_from_non_indian_branches total_borrowings total_borrowings_in_india borrowing_from_RBI borrowing_from_other_banks borrowing_from_other_instiutes total_borrowings_outside_India total_secured_borrowings total_other_liabilities_prov bills_payable interoffice_adjustments interest_accrued subordinate_debt deferred_tax_liabilities other_liabilities_include_prov total_liabilities cash_in_hand balances_with_rbi balances_with_banks_in_india money_at_call_and_short_notice bal_with_banks_outside_ind total_investments investments_in_india investments_outside_india total_advances unsecured_advances total_advances_in_india total_advances_outside_india total_fixed_assets premises fixed_assets_under_construction other_fixed_assets total_other_assets interoffice_adjustments_net interest_accrued_ tax_paid stationery_and_stamps total_assets Net_Interest_Income Provisions_and_Contingencies Net_Profit Income_on_Investments Miscellaneouse_income Total_Income taxation_provisions NPA_provisions investments_provisions other_provisions total_provisions Gross_NPA_Addition_in_year Gross_NPA_Reduction_in_year Gross_NPA_Written_off_in_year Profit_Brought_Forward Real_Estate_Sector_Advances Total_Advances Total_Deposits Total_Borrowings Total_Foreign_Currency_Assets Total_Foreign_Currency_Lia Total_Loans_and_Advances NRI_Deposits Tier_I_Capital_Standalone CET_I_Capital_Standalone Tier_II_Capital_Standalone Total_Capital_Standalone

foreach v of local vars {
    display "Processing variable: `v'"

    * Get original label
    local orig : variable label `v'
    display "Original label: `orig'"

    * Use regex to split into base and suffix (brackets part)
    if regexm("`orig'", "^(.*?)(\\s*\\(.*)?$") {
        local base = regexs(1)
        local suffix = regexs(2)
    }
    else {
        local base = "`orig'"
        local suffix = ""
    ...
    * Replace underscores with spaces
    local cleaned_base : subinstr local base "_" " ", all

    * Count number of words
    local nwords : word count `cleaned_base'

    * Loop through each word and capitalize
    local proper_base ""
    forvalues i = 1/`nwords' {
        local word : word `i' of `cleaned_base'
        local proper_base "`proper_base' `=ustrproper("`word'")'"
    }

    * Trim and de-duplicate spaces
    local proper_base : subinstr local proper_base "  " " ", all
    local newlabel = trim("`proper_base'`suffix'")
    
    display "New label: `newlabel'"

    * Apply the cleaned label
    label variable `v' "`newlabel'"
}

*/

**# Correct Labels for Latex for the &
...
**# Check the interest_accrued_ variable out in the excel 
...
la var NRI_Deposits "NRI Deposits (In Billion Rs.)"
la var Certificates_Deposits "Certificate of Deposits (In Billion Rs.)"
la var Tier_I_Capital_Standalone "Tier I Capital Standalone (In Billion Rs.)"
la var CET_I_Capital_Standalone "CET I Capital Standalone (In Billion Rs.)"
la var Tier_II_Capital_Standalone "Tier II Capital Standalone (In Billion Rs.)"
la var Total_Capital_Standalone "Total Capital Standalone (In Billion Rs.)"
la var Total_RWAs_Standalone_a_b_c "Total Risk Weighted Assets Standalone (In Billion Rs.)"
la var Tot_RWAs_Cred_Risk_Standalone_a "Total Credit Risk Weighted Assets Standalone (In Billion Rs.)"
la var Tot_RWAs_MKt_Risk_Standalone_b "Total Market Risk Weighted Assets Standalone (In Billion Rs.)"
la var Tot_RWAs_Oprt_Risk_Standalone_c "Total Operational Risk Weighted Assets Standalone (In Billion Rs.)"
la var Capital_Ratio_CRAR_Standalone "CRAR Standalone"

la var Food_Credit_Gross_Adv "Food Related Gross Advances (In Billion Rs.)"
la var Food_Credit_GNPAs "Food Related GNPAs (In Billion Rs.)"
la var Food_Credit_GNPA_Ratio "Food Related GNPA Ratio"
la var Non_Food_Credit_Gross_Adv "Non Food Related Gross Advances (In Billion Rs.)"
la var Non_Food_Credit_GNPAs "Non Food Related GNPAs (In Billion Rs.)"
la var Non_Food_Credit_GNPA_Ratio "Non Food Related GNPA Ratio"
la var Agri_Allied_Act_Gross_Adv "Agri-Allied Gross Advances (In Billion Rs.)"
la var Agri_Allied_Act_Gross_NPAs "Agri-Allied GNPAs (In Billion Rs.)"
la var Agri_Allied_Act_GNPA_Ratio "Agri-Allied GNPA Ratio"
la var Industry_Gross_Adv "Industry Gross Advances (In Billion Rs.)"
la var Industry_Gross_NPAs "Industry GNPAs (In Billion Rs.)"
la var Industry_GNPA_Ratio "Industry GNPA Ratio"
la var Services_Gross_Adv "Services Gross Advances (In Billion Rs.)"
la var Services_Gross_NPAs "Services GNPAs (In Billion Rs.)"
la var Services_GNPA_Ratio "Services GNPA Ratio"
la var Retail_Loans_Gross_Adv "Retail Sector Gross Advances (In Billion Rs.)"
la var Retail_Loans_Gross_NPAs "Retail Sector GNPAs (In Billion Rs.)"
la var Retail_Loans_GNPA_Ratio "Retail Sector GNPA Ratio"
la var Other_Non_Food_Cred_Gross_Adv "Other Non Food Related Gross Advances (In Billion Rs.)"
la var Other_Non_Food_Cred_Gross_NPAs "Other Non Food Related GNPAs (In Billion Rs.)"
la var Other_Non_Food_Cred_GNPA_Ratio "Other Non Food Related GNPA Ratio"
la var Retail_Auto_Loans_Gross_Adv "Gross Retail Auto Sector Loans (In Billion Rs.)"
la var Retail_Auto_Loans_GNPAs "Retail Auto Sector GNPAs (In Billion Rs.)"
la var Retail_Auto_Loans_GNPA_Ratio "Retail Auto Sector GNPA Ratio"
la var Retail_Credit_Cards_Gross_Adv "Gross Retail Credit Card Loans (In Billion Rs.)"
la var Retail_Credit_Cards_GNPAs "Retail Credit Card GNPAs (In Billion Rs.)"
la var Retail_Credit_Cards_GNPA_Ratio "Retail Crdeit Card GNPA Ratio"
la var Retail_Edu_Loans_Gross_Adv "Gross Retail Education Loans (In Billion Rs.)"
la var Retail_Edu_Loans_GNPAs "Retail Education Loans GNPAs (In Billion Rs.)"
la var Retail_Edu_Loans_GNPA_Ratio "Retail Education Loans GNPA Ratio"
la var Other_Retail_Loans_Gross_Adv "Other Retail Sector Gross Advances (In Billion Rs.)"
la var Other_Retail_Loans_GNPAs "Other Retail Sector GNPAs (In Billion Rs.)"
la var Other_Retail_Loans_GNPA_Ratio "Other Retail Sector GNPA Ratio"
la var Gross_Advances_p "Total Gross Advances (In Billion Rs.)"
la var Gross_NPAs_p "Total GNPAs (In Billion Rs.)"
la var Gross_NPAs_to_Gross_Adv_perc_p "36. GNPA Ratio"
la var Priority_Sect_Gross_Adv "Priority Sector Gross Advances (In Billion Rs.)"
la var Priority_Sect_Gross_NPAs "Priority Sector GNPAs (In Billion Rs.)"
la var Priority_Sect_GNPA_Ratio "Priority Sector GNPA Ratio"
la var Non_Priority_Gross_Adv "Non-Priority Sector Gross Advances (In Billion Rs.)"
la var Non_Priority_GNPAs "Non-Priority Sector GNPAs (In Billion Rs.)"
la var Non_Priority_GNPA_Ratio "Non-Priority Sector GNPA Ratio"
la var Priority_Agri_Allied_Gross_Adv "Priority Agri-Allied Sector Gross Advances (In Billion Rs.)"
la var Priority_Agri_Allied_Gross_NPAs "Priority Agri-Allied Sector GNPAs (In Billion Rs.)"
la var Priority_Agri_Allied_GNPA_Ratio "Priority Agri-Allied Sector GNPA Ratio"
la var Priority_MSE_Gross_Adv "Priority MSME Sector Gross Advances (In Billion Rs.)"
la var Priority_MSE_GNPA_Ratio "Priority MSME Sector GNPAs (In Billion Rs.)"
la var Priority_MSE_GNPA_Ratio "Priority MSME Sector GNPA Ratio"
la var Other_Priority_Sect_Gross_Adv "Other Priority Sector Gross Advances (In Billion Rs.)"
la var Other_Priority_Sect_Gross_NPAs "Other Priority Sector GNPAs (In Billion Rs.)"
la var Other_Priority_Sect_GNPA_Ratio "Other Priority Sector GNPA Ratio"
la var Total_No_Clerks "Total Number of Clerks Employed"
la var Total_No_Officers "Total Number of Officers Employed"
la var Total_No_Sub_Staff "Total Number of Sub Staff Employed"
la var Total_Employees "Total Number of Employeesed"


save "$clean/Annual_Financials_2005_2024_Financials", replace




clear all
import delimited using "$raw\Market Cap Data.txt", delimiter("|") varnames(1) encoding("utf-8")


rename co_stkdate date


* Step 1: Convert string date to numeric Stata date
* Adjust the format below depending on your actual string format
gen date_num = daily(date, "DMY")   // use "YMD" if format is "2024-03-31"
format date_num %td
...
* Step 3: Keep only March observations
keep if month == 3

rename company_name bank_name 
* Step 4: Tag the latest March date for each bank-year
gen tag = .
bysort bank_name year (date_num): replace tag = 1 if _n == _N


replace bank_name = trim(subinstr(subinstr(subinstr(bank_name, "[MERGED]", "", .), ".", "", .), "[", "", .))
rename equity_bv_on_stkdate equity_book_value

local sto bse_market_cap-equity_book_value 

foreach s of varlist `sto'{
	replace `s'= "." if `s'== "NA"
} 

destring bse_market_cap-equity_book_value, replace 
 
ge MB_Ratio_NSE = nse_market_cap/equity_book_value
la var MB_Ratio_NSE "NSE Market to Book Ratio"
ge MB_Ratio_BSE = bse_market_cap/equity_book_value
la var MB_Ratio_BSE "BSE Market to Book Ratio"




// Standardize bank names for perfect merge
replace bank_name = "AXIS BANK LTD." if bank_name == "AXIS BANK LIMITED" |  bank_name == "AXIS BANK LTD"
replace bank_name = "BANDHAN BANK LTD." if bank_name == "BANDHAN BANK LIMITED" | bank_name == "BANDHAN BANK LTD"
replace bank_name = "CATHOLIC SYRIAN BANK LTD." if bank_name == "CSB BANK LIMITED" | bank_name == "C S B BANK LTD"
replace bank_name = "CENTRAL BANK OF INDIA" if bank_name == "CENTRAL BANK OF INDIA LTD"
replace bank_name = "CITY UNION BANK LTD." if bank_name == "CITY UNION BANK LIMITED" |  bank_name == "CITY UNION BANK LTD" 
replace bank_name = "DCB BANK LTD." if bank_name == "DCB BANK LIMITED" | bank_name == "D C B BANK LTD"
replace bank_name = "DHANALAKSHMI BANK LTD." if bank_name == "THE DHANALAKSHMI BANK LTD" | bank_name == "DHANLAXMI BANK LIMITED" | bank_name == "DHANLAXMI BANK LTD"
replace bank_name = "FEDERAL BANK LTD." if bank_name == "FEDERAL BANK LTD"
replace bank_name = "HDFC BANK LTD." if bank_name == "H D F C BANK LTD"
replace bank_name = "ICICI BANK LTD." if bank_name == "ICICI BANK LIMITED" | bank_name == "I C I C I BANK LTD"
replace bank_name = "IDFC BANK LTD." if bank_name == "IDFC FIRST BANK LIMITED" | bank_name == "I D F C FIRST BANK LTD"
replace bank_name = "ING VYSYA BANK LTD." if bank_name == "ING VYSYA BANK" | bank_name == "ING VYSYA BANK LTD" | bank_name == "I N G VYSYA BANK LTD" 
replace bank_name = "PUNJAB AND SIND BANK" if bank_name == "PUNJAB & SIND BANK"
replace bank_name = "RBL BANK LTD" if bank_name == "R B L BANK LTD"
replace bank_name = "STATE BANK OF BIKANER AND JAIPUR" if bank_name == "STATE BANK OF BIKANER & JAIPUR"
replace bank_name = "YES BANK LTD." if bank_name == "YES BANK LTD"
replace bank_name = "JAMMU AND KASHMIR BANK LTD." if bank_name == "JAMMU & KASHMIR BANK LTD"
replace bank_name = "KARNATAKA BANK LTD." if bank_name == "KARNATAKA BANK LTD"
replace bank_name = "KOTAK MAHINDRA BANK LTD" if bank_name == "KOTAK MAHINDRA BANK LTD."
replace bank_name = "LAKSHMI VILAS BANK LTD." if bank_name == "LAKSHMI VILAS BANK LTD"
...
* Step 5: Keep only the latest observation
keep if tag == 1
drop tag

keep if year > 2004 
keep bank_name year bse_market_cap nse_market_cap equity_book_value MB_Ratio_NSE MB_Ratio_BSE 
order bank_name year bse_market_cap nse_market_cap equity_book_value MB_Ratio_BSE MB_Ratio_NSE

la var bse_market_cap "BSE Market Capitalization"

save "$clean/Market_Cap_Copy", replace

merge 1:1 bank_name year using "$clean/Annual_Financials_2005_2024_Financials", force nogen 

sort bank_name year 
save "$clean/Annual_Financials_2005_2024_All_Vars", replace
